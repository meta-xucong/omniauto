"""JSON sidecar for wxauto4-based WeChat access.

Run this script with the Python 3.12 wxauto4 runtime. The main bridge runs on
the project Python version, while wxauto4 is isolated here to avoid dependency
conflicts.
"""
...
def run_action(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not window_probe["visible_windows"]:
        return {
            "login_window_exists": False,
            "online": False,
            "state": "main_window_not_found",
            "window_probe": window_probe,
            "error": "No visible WeChat main window was found; refusing to start a new WeChat login window.",
        }
    ...
    if args.action == "status":
        ...
        chat_result = safe_call(lambda: wx.ChatWith(args.target, exact=args.exact, force=True, force_wait=1.0))
        time.sleep(0.8)
