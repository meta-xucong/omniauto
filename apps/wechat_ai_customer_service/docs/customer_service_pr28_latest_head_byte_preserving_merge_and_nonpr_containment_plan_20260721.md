...
> 状态：开发方案审计版，仅落文档，尚未修改代码、尚未执行合并。
>
> 适用范围：微信 AI 客服的最新 PR #28（当前审计头 `46a7dd0880e71a0fbd158a9a47d021ac2b7a90fc`）与现有代码的受控合并。
>
...
1. **PR 字节级不变**：PR 最新头的 9 个 PR-owned 文件必须逐文件等于 `git show 46a7dd0:<path>` 的字节内容。不得在 PR 文件中加兼容分支、改测试、改文档、改注释或改换行。
...
- PR：#28
- 最新 head：`46a7dd0880e71a0fbd158a9a47d021ac2b7a90fc`
- PR 父提交：`2120f16744aebe3d8edbdf9c3f407375bfeed279`
...
1. 拉取 `origin/pr-28`，确认其指向 `46a7dd0`；如果 head 变化，停止并重新审计，不得默认跟进新 head。
...
5. 合并后逐文件执行：
   - `git show HEAD:<path>` 与 `git show 46a7dd0:<path>` 字节比较；
   - 搜索冲突标记；
   - `git diff --check`；
   - 相关 Python 文件编译/语法检查。
...
1. 更新 `apps/wechat_ai_customer_service/adapters/wechat_pr28_runtime_adapter.py` 中的 `PR28_HEAD` 与 `PR28_BLOBS`，使其记录 `46a7dd0` 的实际哈希。这个动作只更新版本锁定元数据，不改变 PR 文件。
...
| 同侧等时长语音的 ordinal 锚点漂移 | 不建议 | 需要改 PR 的身份算法；外围只能止血去重，不能保证不重复转写 |
...
2. 9 个 PR-owned 文件与 `46a7dd0` 字节一致；
...
- 将既有 `wechat_pr28_runtime_adapter.py` 的 `PR28_HEAD` 和 `PR28_BLOBS` 更新到 `46a7dd0`，用于锁定最新 PR 的实际 blob。
...
已完成合并提交，并复跑“ancestor + blob”审计：最新 PR head 是当前提交的祖先，9 个 PR-owned 文件的 blob 与 `origin/pr-28` 全部一致。下一步只推送专用分支并创建草稿 PR，供联调和同事审阅；未得到确认前不合并 `main`。
