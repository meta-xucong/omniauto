...
> **当前状态（2026-07-21）：实施记录，当前状态索引已独立收口。** 本文保留合并过程和授权清理记录；当前问题关闭/开放状态以 [PR #28 / Vision 残留问题收口索引](customer_service_pr28_residual_issue_closeout_20260721.md) 为准。
...
## 10. 合并后内部硬化记录（2026-07-21）

在 `main` 合并完成后，已从合并结果建立后续内部硬化分支。该轮只调整既有实现内部逻辑和测试，不改变函数名、字段名、参数、返回形状或模块边界：

- 语音结构绑定不再仅凭“唯一 duration”接受远离原锚点的候选；必须保留局部结构关系。
...
> **状态更正**：上文较早的追加记录仍保留“Sidecar 保留历史空门面”的原始证据，但该结论已被本轮明确授权覆盖。当前代码中该空门面、PR 运行时适配器的两个旧图片事务门面，以及 Vision 文件/裁切/归档兼容入口均已删除；不要按历史段落恢复它们。
