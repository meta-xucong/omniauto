...
该文档没有提供 CRM 客户、线索、微信号、聊天记录、增量变更回调或商品写回接口。因此第一期只同步店铺和车辆域；不得通过 UI 抓取来伪造 CRM 集成。未来如取得官方 CRM API、导出或 Webhook，必须作为独立客户域接入，不与车辆记录混表。
...
实现时新增一个不依赖微信系统的核心包（建议仓库顶层 `packages/dafengche_product_master/`），其内容只包括：大风车签名/只读客户端、无损镜像模型、同步编排、字段策略执行、人工记录与绑定、兼容投影和客户证据查询。包内禁止导入：
...
| `DafengcheTransport` 与密钥提供器 | 发送已签名的只读请求，读取运行时凭据 | 本项目密钥/网络适配器 | 接入自己的 KMS、环境变量和 HTTP 客户端 |
...
当前接口没有官方写入能力，因此同步器只读；管理台对绑定大风车车辆的“编辑”默认只允许编辑本地扩展或创建人工覆盖，不得假装已回写大风车。
...
仅在拿到官方授权接口后，按同样的无损镜像原则新增客户/线索独立存储、PII 策略和会话绑定；不得为了“账号功能已开通”而复用车辆表或抓取 UI。
...
| 产物 | 职责 | 明确不做的事 |
| --- | --- | --- |
| `packages/dafengche_product_master/` | 可复制的无宿主核心包：签名只读客户端、v2 无损镜像、人工车辆、内存仓储端口、字段策略和客户证据投影 | 不导入微信、Brain、RPA、管理后台、项目路径或数据库驱动 |
...
| `tests/fixtures/dafengche/` 与 `run_dafengche_product_master_checks.py` | 脱敏四接口夹具、签名、无损保留、脱敏、过期、跨店、手工录入、兼容、Brain/RPA 边界回归 | 不包含真实账号、密钥、车主、客户或真实 VIN/车牌 |
...
- 不执行任何大风车写回、CRM 同步或生产凭据配置，直到相应官方合同和授权到位。
