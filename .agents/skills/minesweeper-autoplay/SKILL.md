...
description: 运行、长测、诊断和优化 Windows 扫雷自动游玩流程。用于用户要求自动打开扫雷、自动玩、跑单局或多局测试、监控长测、分析失败截图、排查识图/几何/点击/策略问题、或继续迭代 `workflows/temporary/desktop/minesweeper_solver.py` 时。
---
...
- Main solver: `../../../workflows/temporary/desktop/minesweeper_solver.py`
- Runtime artifacts: `../../../runtime/test_artifacts/verification/minesweeper/`
- Knowledge closeout runs: `../../../runtime/knowledge_runs/`
- Runner script: `scripts/run-solver.ps1`
...
2. Start the solver with `scripts/run-solver.ps1` unless there is a good reason to call the Python entry directly.
...
5. When changing the solver, patch `../../../workflows/temporary/desktop/minesweeper_solver.py`, run `py_compile`, then rerun a targeted smoke test before longer tests.
6. Keep the solver in its current workflow path unless the user explicitly asks for a code move or platformization step.
...
1. Read `../../../runtime/test_artifacts/verification/minesweeper/solver_stop_summary.txt` when present.
