...
任务: 猫砂 | 站点: 1688 | 页数: 3 | 排序: price_asc
"""
...
# 配置常量
# ------------------------------------------------------------------
KEYWORD_GBK = quote("猫砂", encoding="gbk")
BASE_URL = (
...
MAX_PAGES = 3
SAMPLE_SIZE = 5
OUTPUT_DIR = Path("runtime/data/reports/1688_cat_litter_50")
...
async def step_warmup(ctx: TaskContext):
    ...
    if await is_login_page(browser.page) or await is_captcha_page(browser.page):
        _safe_print("[Step 0] 首页触发验证/登录，等待手动处理...")
...
async def step_search(ctx: TaskContext):
    browser: StealthBrowser = ctx.browser_state["browser"]
    _safe_print(f"[Step 1] 访问搜索页: {BASE_URL}&beginPage=1")
    await browser.goto(f"{BASE_URL}&beginPage=1", wait_until="domcontentloaded", timeout=20000)
    await browser.wait_for_selector(".search-offer-item, .offer-item", timeout=15000)
    await browser.simulate_human_viewing()
    return {"success": True, "url": browser.page.url}
...
async def main():
    ...
    ctx = TaskContext(task_id="1688_cat_litter_50", browser_state={"browser": browser})
