...
def extract_customer_data(text: str, required_fields: list[str] | None = None) -> CustomerExtraction:
    ...
    # A phone number plus any other business field is enough to consider this a
    # customer-data message, even if a required field is missing.
    business_fields = {"name", "phone", "address", "product", "quantity", "spec", "budget"}
    is_customer_data = len(business_fields.intersection(fields)) >= 2 or bool(fields.get("phone"))
