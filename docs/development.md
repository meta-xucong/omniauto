...
## 9. 实战经验与架构决策（从 1688 实测中提炼）

### 9.1 脚本生成：模板模式 vs 原始代码模式

- **模板模式**（`omniauto.templating.generator`）适合结构固定、步骤可预定义的任务（如电商商品调研）。优势是确定性高、可复用、维护成本低。
- **复杂任务**（涉及 Excel/Word 导出、条件判断、循环、非标准 DOM 提取）模板模式无法覆盖。此时应让外部 AI（Codex / Claude）直接生成完整 Python 代码并执行。
- **当前实践**：复杂且高频的任务（如 1688 商品调研）已被固化为 Jinja2 模板（见 `src/omniauto/templates/workflows/`）；一次性灵活任务走原始代码生成通道。

### 9.2 1688 反爬实战策略

经过多轮实测验证，1688 的防御层级与有效应对方案如下：

| 层级 | 1688 防御 | 实测有效方案 |
|------|-----------|-------------|
| L1 透明层 | 检测无头浏览器 | StealthBrowser 反检测参数 + 真实 Chrome Profile |
| L2 交互层 | 搜索框 JS 动态加载 | 绕过：直接构造搜索 URL (`s.1688.com/selloffer/...`) |
| L3 身份层 | **强制登录**（未登录跳转淘宝登录页） | **使用已登录的 Chrome Profile (`user_data_dir`)** |
| L4 验证层 | 滑块验证码 | 浏览器级 API 失效，触发 `AuthManager` 人工介入或切换视觉模式 |

**核心结论**：
1. 1688 搜索功能**强制要求登录态**。无登录 Cookie 时访问搜索 URL 会被 302 重定向到淘宝登录页。
2. **最有效的方案是连接已登录的真实 Chrome Profile**。只要 Profile 中保持 1688/淘宝登录状态，即可稳定抓取商品列表和详情。
3. 为应对 Profile 被风控的情况，实现了 **动态 Profile 旋转**：当连续 2 个详情页触发验证码/登录拦截时，自动删除当前 Profile，生成新的临时 Profile，重新预热后恢复任务。
4. DOM 选择器必须足够宽泛（同时兼容 PC 版和 H5 版），避免依赖单一易变的 class 名。详情页 URL 优先使用 `detail.m.1688.com/page/index.html?offerId={id}`，桌面浏览器会自动重定向到 PC 详情页，同时该链接结构更稳定。

### 9.3 浏览器行为约束（实测有效）

- `goto()` 前不再预热鼠标或滚动，防止验证码滑块坐标漂移。
- 列表页与详情页之间必须插入 `cooldown(5.0, 10.0)` 级别的随机等待。
- 翻页使用 `throttle_request(4.0, 8.0)` 控制请求节奏。
- 标题提取需显式过滤掉 "找相似" 等按钮文本，防止数据污染。
