...
```bash
uv run omni generate "访问百度并搜索关键词" --output scripts/my_task.py
```

生成的脚本位于 `scripts/my_task.py`，可直接查看和修改。

### 3.3 校验脚本安全性

```bash
uv run omni validate scripts/my_task.py
```
...
```bash
uv run omni run --script scripts/my_task.py --headless
```
...
- **StealthBrowser（隐形浏览器）**：基于 Playwright，注入反检测脚本，支持真实 Chrome Profile
...
```bash
# 生成脚本
omni generate "任务描述" --output ./scripts/task.py

# 校验脚本
omni validate ./scripts/task.py

# 执行脚本（支持断点续传）
omni run --script ./scripts/task.py --headless --task-id my_task_001
...
- [x] 开发 AI 脚本生成器（基于模板 + TaskPlanner）
...
- [`development.md`](./development.md) — 开发架构与实施指南
- [`agents.md`](./agents.md) — AI 开发助手行为规范
