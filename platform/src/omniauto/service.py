...
class OmniAutoService:
    ...
    async def run_workflow(
        ...
        entrypoint: str = "service.run_workflow",
        force_knowledge_closeout: bool = False,
    ) -> Dict[str, Any]:
        script_path_resolved = str(Path(script_path).resolve())
        if not Path(script_path_resolved).exists():
            raise FileNotFoundError(f"脚本不存在: {script_path}")

        requested_task_id = task_id or Path(script_path_resolved).stem
        task_run = self.knowledge.start_task_run(
            script_path=script_path_resolved,
            task_id=requested_task_id,
            entrypoint=entrypoint,
            description=task_description,
            force=force_knowledge_closeout,
        )
        ...
        try:
            ...
            result_payload["knowledge_closeout"] = self.knowledge.finalize_task_run(
                task_run,
    ...
    def schedule_task(
        ...
        async def _job():
            await self.run_workflow(
                ...
                entrypoint="service.schedule_task",
            )
