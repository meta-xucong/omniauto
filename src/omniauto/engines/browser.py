...
class StealthBrowser:
    ...
    async def start(self) -> "StealthBrowser":
        ...
        if system_chrome:
            # Playwright 推荐用 channel='chrome' 连接系统安装的 Chrome
            launch_args["channel"] = "chrome"

        # 上下文配置：统一指纹、时区、地理位置以匹配中国用户环境
        # headful 模式下 viewport=None 配合精确的 --window-size，可避免 Windows DPI 缩放导致的画面比例失调
    ...
    async def _inject_stealth_scripts(self) -> None:
        """注入 Stealth 脚本覆盖检测属性（在 context 级别注入，确保新页面/iframe 均生效）."""
    ...
    async def goto(self, url: str, wait_until: str = "domcontentloaded", timeout: Optional[int] = None) -> None:
        """导航到指定 URL. 若检测到登录页且开启了 auto_handle_login, 会自动处理登录态.
    ...
    async def click(self, selector: str, delay: Optional[tuple[float, float]] = None) -> None:
        """点击元素，模拟人类鼠标移动轨迹和随机延迟。若标准 API 因可见性失败，降级到 DOM 操作."""
        if self._page is None:
    ...
    async def type_text(
        ...
        try:
            ...
            # 使用 Playwright 的 type 但传递随机延迟（每字符 50~250ms）
