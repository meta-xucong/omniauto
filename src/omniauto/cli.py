...
async def _run_script(script_path: str, headless: bool, task_id: Optional[str] = None) -> None:
    ...
    spec = importlib.util.spec_from_file_location("atomic_script", script_path)
    module = importlib.util.module_from_spec(spec)
    sys.modules["atomic_script"] = module
    spec.loader.exec_module(module)

    if not hasattr(module, "workflow"):
        click.echo("❌ 脚本中未找到 'workflow' 变量", err=True)
        sys.exit(1)
    ...
    try:
        ...
        click.echo(f"[DONE] 工作流结束，最终状态: {final_state.name}")
        if context.outputs:
            click.echo("[DATA] 输出数据:")
            for k, v in context.outputs.items():
                click.echo(f"  {k}: {v}")
        logger.info("workflow_finished", task_id=workflow.task_id, state=final_state.name)
    except Exception as exc:
        click.echo(f"[ERR] 执行异常: {exc}", err=True)
